...
class AllReviewsTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return FirebaseManager.current.campaigns!.count
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let new = tableView.dequeueReusableCell(withIdentifier: "CustomNewReviewTableViewCell", for: indexPath) as? CustomNewReviewTableViewCell {
            new.adDescription.text = FirebaseManager.current.campaigns![indexPath.row].description
            new.title.text = FirebaseManager.current.campaigns![indexPath.row].name
            new.userEmail.text = FirebaseManager.current.campaigns![indexPath.row].user_id
            let verified = FirebaseManager.current.campaigns![indexPath.row].verification_status
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        print(FirebaseManager.current.campaigns![indexPath.row])
    }
